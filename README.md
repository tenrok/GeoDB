...
where
	n.ip <= (select long from ip2long i where i.long != 0)
order by ip desc
limit 1;
```
#### Пример 2
...
from networks n
	...
	left join cities     g on g.id = n.city_id
where n.ip <= (
	(substr(:ip, 1, instr(:ip, '.') - 1) * 16777216) +
	(substr(:ip, instr(:ip, '.') + 1, instr(substr(:ip, instr(:ip, '.') + 1), '.') - 1) * 65536) +
	(substr(:ip, instr(:ip, '.') + instr(substr(:ip, instr(:ip, '.') + 1), '.') + 1, instr(substr(:ip, instr(:ip, '.') + instr(substr(:ip, instr(:ip, '.') + 1), '.') + 1), '.') - 1) * 256)
)
order by n.ip desc
limit 1;
```
